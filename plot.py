...
data_16_vec_storage = np.loadtxt("spikes_16_vec_storage.csv", delimiter=",", skiprows=1)
...
axes[3].scatter(data_16_vec_storage[:,0], data_16_vec_storage[:,1],s=1)
...
print(f"16-bit vec storage average spikes per neuron={np.average(np.bincount(data_16_vec_storage[:,1].astype(int)))}")
plt.show()
